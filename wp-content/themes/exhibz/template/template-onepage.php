...
if ( isset( $menu_locations[ 'primary' ] ) ) :
	...
	foreach ( $menu_items as $item ) {
		if ( ($item->object == 'page') && (exhibz_get_post_meta( exhibz_main( $item->object_id, false ), 'xs_page_section' ) == true) && ($item->object_id != $self_id) )
			$menu_items_include[]	 = $item->object_id;
		$menu_items				 = count( $menu_items_include );
	}

	$query = new WP_Query( array( 'post_type' => 'page', 'post__in' => $menu_items_include, 'posts_per_page' => $menu_items, 'orderby' => 'menu_order', 'order'=> 'ASC' ) );
