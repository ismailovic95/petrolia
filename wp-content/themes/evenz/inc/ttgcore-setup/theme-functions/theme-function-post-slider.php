...
if(!function_exists( 'evenz_template_post_slider' )){
	function evenz_template_post_slider( $atts = array() ){
		...
		if ( $wp_query->have_posts() ) : 
			...
			?>
			<div id="<?php echo esc_attr( $list_id ); ?>" class="evenz-slider__container">
				<div class="evenz-slider__main">
					...
					<div class="evenz-slider__arrows">
						<?php  
						$n = 1;
						foreach ($all_posts as $mypost ){
							?>
							<label for="slides_<?php echo esc_attr( $n ); ?>" class="<?php echo esc_attr( ( $n == 1 )? 'evenz-goto-first' : '' ). esc_attr( ( $n == $total )? 'evenz-goto-last' : '' ) ?>"></label>
							<?php 
							$n++;
						}
						?>
					</div>
					<div class="evenz-slider__nav">
						<div>
							<?php  
							$n = 1;
							foreach ($all_posts as $mypost ){
...
if(function_exists('ttg_custom_shortcode')) {
	ttg_custom_shortcode("qt-post-slider","evenz_template_post_slider");
}
 ...
 */
add_action( 'vc_before_init', 'evenz_template_post_slider_vc' );
if(!function_exists('evenz_template_post_slider_vc')){
	function evenz_template_post_slider_vc() {
  		vc_map( 
  			array(
				"name" => esc_html__( "Post slider", "evenz" ),
				"base" => "qt-post-slider",
				"icon" => get_theme_file_uri( '/inc/ttgcore-setup/theme-functions/img/post-slider.png' ),
				"description" => esc_html__( "Responsive slideshow", "evenz" ),
